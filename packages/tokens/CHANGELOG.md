...
- d6d8d57: Focus shadows are now theme-aware. `--ds-shadow-focus` moved from the primitive layer to the semantic layer, so the dark theme finally gets its lighter focus ring instead of the light-theme cobalt. New `--ds-shadow-focus-danger` token for invalid-control focus states.
