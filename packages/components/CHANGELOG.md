...
- ea7b44c: The "+n" tile on `ds-select` and `ds-searchable-select` can now show what it is
...
- 2fd687a: `SlotPresenceController` now records presence against the slot that listened
...
- 0723abf: **Breaking:** `ds-checkbox` and `ds-radio` take their value as `checkbox-value`
...
- 0723abf: `announce-on-connect` works as an attribute on `ds-alert`. Lit derives the
...
- 0723abf: `max-lines` works as an attribute on `ds-select` and `ds-searchable-select`.
...
- 6856341: The "+n" tile counts hidden tiles from the first tile rather than from the
...
- c4fd9da: Stop `ScrollFadeController` leaving work behind when its host goes away. The controller retries attaching to a scroller that has not rendered yet, up to ten animation frames, but the retry handle was never stored, so `#detach` could not cancel it: a dialog opened and closed quickly left the rest of the chain running against a host that was gone.
...
- 7c598fc: Keep the PageShell mobile menu button square by applying its configured size to both dimensions.
- b4339d6: Keep StatTile the same height while its value loads. The value row floored itself with `calc(font-size * line-height)`, but the line-height tokens resolve to lengths rather than ratios, so the multiplication was invalid and the declaration was dropped - the row measured 28px around the skeleton and 36px around the loaded value. It now reserves one line box.

## 0.69.1

### Patch Changes

- 09d6bec: A toast now reads as its own surface. It filled with `--ds-color-bg`, the same
...
- 8696dca: Render crisply on 1x displays.
...
- 43555fd: Center short values in `ds-table` stacked cells. A value shorter than a line box - an icon scale, a rating - was pinned to the top of its row and read as lifted next to its label. Stacked cell content now reserves one line box, so a short value centers on the label's line while wrapped text still starts on the label's first line.
...
- Updated dependencies [4dd80f2]
  - @jsekulowicz/ds-tokens@0.67.0

## 0.66.0

### Minor Changes

- 1bbb277: `ds-button` accepts `href`, `target` and `rel`, rendering an `<a part="button">` instead of a `<button>`.
...
- 54cdec7: Tighten the reserved field-message row, and give `ds-checkbox` one.
...
- Updated dependencies [f569725]
  - @jsekulowicz/ds-tokens@0.61.0
  - @jsekulowicz/ds-core@0.61.0
...
- 60f56e4: Fix a render loop that froze the tab when scrolling a listbox whose rows aren't all the same height.
...
- 9fb3417: Let a select option render its label as a badge, in the listbox and on the selected tile.
...
- 76478d3: Move the character counter into the label row, and let a dialog override the width its size sets.
...
- e8aafec: Fix `ds-fieldset` laying its controls out in a column, and expose the row as a part.
...
- 3401aec: Stop the loading state from resizing `ds-button`, close nested modals with their container, and add `ds-fieldset`.
...
- c17f71a: `ds-select` and `ds-searchable-select`: show the listbox before restoring its
  scroll position.
...
- 4908a55: `ds-alert`'s dismiss button now uses `ds-button` and `ds-icon` instead of a raw
