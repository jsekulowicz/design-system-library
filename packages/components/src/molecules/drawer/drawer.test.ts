...
const TEMPLATE = `
  <ds-drawer side="end">
    ...
    <p>Tune the list to your taste.</p>
    <ds-button slot="footer" variant="ghost">Reset</ds-button>
  </ds-drawer>
`;

function getDialogEl(host: DsDrawer): HTMLDialogElement {
...
describe('<ds-drawer>', () => {
  it('renders title, default and footer slots inside ds-card', async () => {
    ...
    expect(titleSlot.assignedElements()[0]?.textContent).toBe('Filters');
    expect(defaultSlot.assignedElements()[0]?.tagName.toLowerCase()).toBe('p');
    expect(footerSlot.assignedElements()).toHaveLength(1);
  });

  it('defaults to side=start and size=sm', async () => {
    const el = await mount<DsDrawer>(`<ds-drawer><span slot="title">x</span></ds-drawer>`);
  ...
  it('emits ds-close with returnValue when close(value) is called', async () => {
    ...
    expect(closes[0]?.detail.returnValue).toBe('applied');
  });
  ...
  it('ignores backdrop click when not dismissible', async () => {
    const el = await mountWithProps<DsDrawer>(TEMPLATE, { open: true, dismissible: false });
    const dialog = getDialogEl(el);
    dialog.dispatchEvent(new MouseEvent('click', { bubbles: true, composed: true }));
    await el.updateComplete;
    expect(el.open).toBe(true);
  ...
  it('falls back to aria-label when label prop is set', async () => {
    const el = await mountWithProps<DsDrawer>(TEMPLATE, { label: 'Filter panel' });
    const dialog = getDialogEl(el);
    expect(dialog.getAttribute('aria-label')).toBe('Filter panel');
