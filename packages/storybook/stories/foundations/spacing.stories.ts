    ...
    <section style=${PAGE}>
      <header style="display:grid;gap:var(--ds-space-1)">
        ...
        <p style="margin:0;color:var(--ds-color-fg-muted)">
          Every step is a multiple of 4px. The number in the token name is the step count on that 4px grid, so
      ...
      <div role="table" style=${SCALE_ROW}>
        ...
        ${STEPS.map(
          (s) => html`
            ...
            <span role="cell" style="font-variant-numeric:tabular-nums;color:var(--ds-color-fg-muted)"> ${s.rem} </span>
  ...
  render: () => html`
    <section style="display:grid;gap:var(--ds-space-4);font-family:var(--ds-font-body);color:var(--ds-color-fg)">
      <p style="margin:0;max-width:60ch;color:var(--ds-color-fg-muted)">
        Compare how common steps feel when used as padding. Pair <code>space-3</code> with <code>space-4</code> or
        <code>space-6</code> for dense/comfortable/spacious densities.
      </p>
      ${[2, 3, 4, 6, 8].map(
        (n) => html`
