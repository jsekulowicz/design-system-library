...
const RELABELED_ATTRIBUTE = 'data-ds-docs-relabeled';
const LABEL_CELL_ATTRIBUTE = 'data-ds-docs-label-cell';
const LABEL_WRAPPER_ATTRIBUTE = 'data-ds-docs-label-wrapper';
const LABEL_ATTRIBUTE = 'data-ds-docs-label';
...
function ensureDocsRelabelStyles(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    [${RELABELED_ATTRIBUTE}="true"] [${LABEL_WRAPPER_ATTRIBUTE}="true"] {
      justify-content: center !important;
    }

    [${RELABELED_ATTRIBUTE}="true"] [${LABEL_CELL_ATTRIBUTE}="true"] {
      font-size: 0 !important;
      line-height: 0 !important;
    }

    [${RELABELED_ATTRIBUTE}="true"] [${LABEL_CELL_ATTRIBUTE}="true"]::after {
...
function clearRecentDocsRelabeling(scope: ParentNode = document): void {
  const relabeledItems = scope.querySelectorAll<HTMLElement>(`[${RELABELED_ATTRIBUTE}="true"]`);
  for (const item of relabeledItems) {
    const labelCell = item.querySelector<HTMLElement>(`[${LABEL_CELL_ATTRIBUTE}="true"]`);
    if (labelCell) {
      labelCell.parentElement?.removeAttribute(LABEL_WRAPPER_ATTRIBUTE);
      labelCell.removeAttribute(LABEL_CELL_ATTRIBUTE);
      labelCell.removeAttribute(LABEL_ATTRIBUTE);
    ...
    item.removeAttribute(RELABELED_ATTRIBUTE);
  }
}
...
function relabelRecentDocs(scope: ParentNode = document): void {
  ...
  for (const item of scope.querySelectorAll<HTMLElement>(SEARCH_RESULT_SELECTOR)) {
    if (item.getAttribute(RELABELED_ATTRIBUTE) === 'true') {
      continue;
    }
    ...
    result.cell.setAttribute(LABEL_ATTRIBUTE, result.label);
    result.cell.setAttribute(LABEL_CELL_ATTRIBUTE, 'true');
    result.cell.parentElement?.setAttribute(LABEL_WRAPPER_ATTRIBUTE, 'true');
    item.setAttribute(RELABELED_ATTRIBUTE, 'true');
  }
}
