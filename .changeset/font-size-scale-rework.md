...
- Body and control text now uses `md` (16px) by default. Buttons, text fields, text areas, selects, searchable selects, radios, checkboxes, tabs, nav items, menu items and select options render at 16px regardless of `size` — the previous per-size font shrinking on controls is gone (a `size="sm"` button/input is no longer 12–13px).
- Alert and toast titles bump to `lg` to stay above the larger body text.
- Labels, descriptions, helper/meta text, tables and breadcrumbs keep their smaller sizes for density.
