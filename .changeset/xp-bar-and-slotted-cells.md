...
- `ds-progress-bar`: the label is now bare text (no chip) with a **crisp `--ds-color-bg` outline** (`-webkit-text-stroke` + `paint-order`) so it stays legible over both the filled and empty track; inline icons aren't stroked, so give them their own contrast (a badge/pill). A single `--ds-progress-color` custom property (default `--ds-color-accent`) recolours the track border, the filled indicator and the label together; the empty track is `--ds-color-bg`. The default label size is now `sm` (was `xs`).
